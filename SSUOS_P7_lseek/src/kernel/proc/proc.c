...
void lseek_proc(void *aux , void *filename)
{
	...
	if (fd < 0 )return;
	if (!strcmp((char *)aux, "e"))
	{
		printk("write(fd , \"ssuos \",6)\n");
		write(fd , "ssuos ",6);
		printk ("lseek(fd, 3, SEEK_CUR, e) -> pos %d\n", lseek(fd, 3, SEEK_CUR, aux)); 
		printk("file size %d\n", cur_process->file[fd]->inode->sn_size); 
		lseek(fd, 0, SEEK_SET, NULL);		
		read(fd, buf, BUFSIZ);
		printk("%s\n", buf);
	}
	else if (!strcmp((char *)aux, "a"))
	{
		printk("write(fd , \"ssuos \",6)\n");
		write(fd , "ssuos ",6);
		printk("lseek(fd, 3, SEEK_SET, NULL) -> pos %d\n", lseek(fd, 3, SEEK_SET, NULL)); 
		printk("lseek(fd, 4, SEEK_CUR, a) -> pos %d\n", lseek(fd, 4, SEEK_CUR, aux));
		printk("file size %d\n", cur_process->file[fd]->inode->sn_size); 
		lseek(fd, 0, SEEK_SET, NULL);			
		read(fd, buf, BUFSIZ);
		printk("%s\n", buf);
	}
	else if (!strcmp((char *)aux, "re"))
	{
		printk("write(fd , \"ssuos \",6)\n");		
		write(fd , "ssuos ",6);
		printk("lseek(fd, -3, SEEK_SET, re) -> pos %d\n", lseek(fd, -3, SEEK_SET, aux));
		printk("file size %d\n", cur_process->file[fd]->inode->sn_size); 
		lseek(fd, 0, SEEK_SET, NULL);	
		read(fd, buf, BUFSIZ);
		printk("%s\n", buf);
	}
	else if (!strcmp((char *)aux, "c"))
	{
		printk("write(fd , \"ssuos \",6)\n");
		write(fd , "ssuos ",6);				
		printk("lseek(fd, -4, SEEK_SET, c) -> pos %d\n", lseek(fd, -4, SEEK_SET, aux));
		printk("lseek(fd, 3, SEEK_END, c) -> pos %d\n", lseek(fd, 3, SEEK_END, aux));
	}
	else
	{
		write(fd , "ssuos ",6);
		...
		printk ("%d \n", lseek(fd, -3, SEEK_CUR, aux));  
		write(fd, "world",5);
		lseek(fd, 0, SEEK_SET, aux);
		read(fd , buf, 8);
		printk("%s\n", buf);
		lseek(fd, -9, SEEK_END, aux);
		read(fd, buf, 9);
		printk("%s\n", buf);
	}
